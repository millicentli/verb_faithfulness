...
DATASET_CACHE_PATH = os.environ.get(
    "VEC2TEXT_CACHE", "/projects/berb/mli23/.cache/inversion"
)
...
class Experiment(abc.ABC):
    ...
    def dataset_kwargs(self) -> Dict[str, str]:
        return {
            ...
            "embeddings_from_layer_n": str(self.model_args.embeddings_from_layer_n),
        }
...
class InversionActivationsExperiment(InversionExperiment):
    @property
    def trainer_cls(self):
        return vec2text.trainers.InversionActivationsTrainer

    @property
    def _wandb_project_name(self) -> str:
        return "emb-inv-activations-1"
    
    def load_model(self) -> transformers.PreTrainedModel:
        return InversionActivationsModel(
            config=self.config,
        )
...
EXPERIMENT_CLS_MAP = {
    ...
    "inversion_activations": InversionActivationsExperiment,
