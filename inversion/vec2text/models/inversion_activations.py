...
class InversionActivationsModel(transformers.PreTrainedModel):
    """For inverting model hiddens/activations"""
    ...
    def __init__(self, config: InversionConfig):
        ...
        if config.bf16:
            encoder_decoder = encoder_decoder.bfloat16()
        ...
        if embedder_model_api:
            ...
            if hasattr(embedder.model.config, "text_config"):
                self.embedder_dim = embedder.model.config.text_config.hidden_size
            elif hasattr(embedder.model.config, "hidden_size"):
                self.embedder_dim = embedder.model.config.hidden_size
            else:
                raise ValueError(
    ...
    def call_embedding_model(
        ...
        if self.embedder_fake_with_zeros:
            ...
            sentences = self.embedder_tokenizer.batch_decode(input_ids, skip_special_tokens=True)
            with embedder.trace(sentences) as tracer:
                if hasattr(embedder, "model"):
                    embeddings = (
                        embedder.model.layers[self.embeddings_from_layer_n]
                        .output[0][:, -1, :]
                        .save()
                    )
                else:
                    embeddings = (
                        embedder.transformer.h[self.embeddings_from_layer_n]
                        .output[0][:, -1, :]
                        .save()
                    )
            # Set the current instance's device + dtype to float32
            embeddings = embeddings.to(torch.cuda.current_device())
    ...
    def embed_and_project(
        ...
        if self.embedding_transform_strategy == "repeat":
            ...
            try:
                ...
                # Cast the embed back to float32
                repeated_embeddings = self.embedding_transform(embeddings.to(torch.float32))
